    ...
    @if($infos->count() == 0)
        @can('info_create')
            <div style="margin-bottom: 10px;" class="row">
                <div class="col-lg-12">
                    <a class="btn btn-success" href="{{ route("admin.info.create") }}">
                ...
                <tbody>
                    @foreach($infos as $key => $info)
                        <tr data-entry-id="{{ $info->id }}">
                            ...
                            <td>
                                @can('info_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.info.edit', $info->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan
                            </td>
...
@can('info_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.info.massDestroy') }}",
...
@endcan
  ...
  $('.datatable-info:not(.ajaxTable)').DataTable({ buttons: dtButtons })
